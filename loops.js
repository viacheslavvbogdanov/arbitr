...
//             let tickers = await exchange.fetchTickers() // Not all exchanges supports 'get all in once'
//             let responseTime = Date.now()-startTime
//             d.stats.push({exchange:name,responseTime:responseTime})
//             // log( name.blue.bgLightGray, _tab, propsCount(markets), _tab, (responseTime/1000).toFixed(3)+'ms' )
//             // // print tesponse times
//             // _.each(_.sortBy(d.stats, 'responseTime'), (stat)=>{
//             //     log(stat.responseTime, stat.exchange.darkGray )
//             // })
...
//                 console.time(timeLoopsCalc)
//                 d.markets[name] = await exchange.loadMarkets()
//                 await delay(1000)
//                 const graph = ants.createGraph(d.markets[name])
//                 d.graph[name] = graph
//                 d.loops[name] = ants.findLoops(graph, 3)
//                 console.timeEnd(timeLoopsCalc)
//             }
//             const markets = d.markets[name]
//
//             const loops = d.loops[name]
//             const allChains = ants.findChains(name, markets, loops, tickers)
//             //Filter by profit and volume
//             const unsortedChains =
//                 ants.filterChains(allChains, 0.001, 5, 'ETH')
//             const chains = _.sortBy(unsortedChains, 'profit' )
//
//             if (chains.length>0) {
//                 log(' ')
